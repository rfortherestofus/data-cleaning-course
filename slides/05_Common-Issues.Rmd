...
So, data organization good practices suggest not leaving empty cells, and also being aware of how we encode missing data. Empty rows and columns are an extension of this. Let's see how we can sort them out.
...
It's quite common to find empty rows or columns when we import files, and the reasons for them being in the data can vary. 
...
In smaller datasets we can simply look at the data and see which rows or columns are empty. With larger volumes of data this is not possible, so we can existing functions to identify them.  
...
We can add a unique identifier to each row before subsetting the data to keep only the empty rows. The row id to column function from the package tibble takes the row numbering inherent to the data and places it as a column. 
...
???
To remove the empty rows, we follow the same process as before, but instead of keeping the empty rows, we discard them by negating the is.na condition with an exclamation mark. 
...
Using remove empty and specifying rows, we get the same result as before, and because we set the quiet argument to FALSE, a small message tells us that five empty rows were removed out of twelve in total. This looks much better.
...
Like we did we columns, we can negate the condition for subsetting the columns and this will remove the empty ones.
...
The syntax is simple, we specify that we're removing columns, and if quiet is set to false, then we'll get a message telling us how many and which ones were removed. 
---
...
In this lesson, we'll learn how to make numeric values in our data useable, because they often aren't.
...
A fundamental property in R is that vectors must have all their values of the same mode, which can be character, numeric, or logical.
...
We cannot perform numeric operations on numbers that have non-digits. This excludes decimals though. To get rid of non digits, we have two options as well.
...
Our first approach should be to just parse all the columns in the object with the type convert function from readr. When we do this, we can see that Rank column is now a variable of type double, which is a class of numeric values, the other one is for integers. 
