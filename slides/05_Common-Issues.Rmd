...
cafeteria %>% untangle2("Drinks$", Item, drink_type) %>% 
  clean_names() %>% group_by(drink_type) %>% 
...
## My turn
.large[
- Load the `primates2017` dataset bundled with 📦 .rrured.b[`unheadr`] and create a new column that groups the different species by geographic region. 
]
---
class: inverse
## Your turn
.large[
- Load the `primates2017` dataset bundled with 📦 .rrured.b[`unheadr`] and create a new column that groups the different species by taxonomic family.  
]
...
```{r, eval=TRUE}
pizza_orders %>% get_dupes(Address,starts_with("Cit")) 
```
---
...
.large[
- Load the messy Age of Empires units dataset bundled with .rrured.b[`unheadr`] (AOEunits_raw) and discard units that are not of Type "Archer".
...
.large[
- Load the messy Age of Empires units dataset bundled with .rrured.b[`unheadr`] (AOEunits_raw) and keep only units of Type "Cavalry".
...
Match _trailing_ or _leading_ half of the value with a .orange.b[regexp] and 📦 .rrured.b[`unheadr`]
]  
.pull-left[
Fast  
**food**  
...
.large[Negate the condition and discard empty rows]  
```{r, eval=TRUE}
universities %>% filter(!across(everything(), is.na))
```
...
.large[Coerce variables to `numeric`]
