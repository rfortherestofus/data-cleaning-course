...
In previous lessons, we defined data cleaning and learned how to organize our data. Now we can get started with using R for data cleaning, and in particular, on first getting our data into sensible, usable structures.

Let’s begin by going over the tools and packages that we’ll be using for this part and throughout this course in general.
...
First we can use nested code, in which function calls are nested inside more function calls so that the results get evaluated from the inside out.
...
We can also assign objects with intermediate results that need to be passed to the next function in our sequence of operations.
...
Alternatively, we can use pipes to avoid nested function calls or having so assign results to intermediate objects that we don’t really need.
...
And remember, the pipe operator gets loaded by the tidyverse functions that we’ll be using. 
...
First, by chaining our operations with pipes we pass intermediate results onto the next function, so we don’t need nested code or to assign intermediate results to objects we won’t use again.
...
First we load the magrittr package, take the grades object, pass it  to the mean function, and then pass this result to the round function. Note how we don’t have to write the first argument in the function that are on the right hand side of a pipe. 
